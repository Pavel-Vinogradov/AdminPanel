...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('dashboard.articles.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Создать статью
            </a>
        </div>
        ...
        <div class="overflow-x-auto bg-white shadow rounded-lg">
            <table class="min-w-full table-auto">
                ...
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-500">Просмотры</th>
                ...
                @foreach ($articles as $article)
                    <tr class="border-b">
                        ...
                        <td class="px-6 py-3 text-sm text-gray-800">{{ $article->views }}</td>
                        <td class="px-6 py-3 text-sm text-gray-800">{{ $article->created_at->format('d-m-Y') }}</td>
                        <td class="px-6 py-3 text-sm text-gray-800">
                            <a href="{{ route('dashboard.articles.edit', $article->id) }}" class="text-blue-600 hover:text-blue-800">Редактировать</a> |
                            <form action="{{ route('dashboard.articles.destroy', $article->id) }}" method="POST" class="inline" onsubmit="return confirm('Вы уверены, что хотите удалить эту статью?');">
                                @csrf
                                @method('DELETE')
